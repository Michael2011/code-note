...
style = vgg.preprocess(np.asarray(Image.open(args.style_image).convert('RGB').resize((512,512)), dtype=np.float32))
...
for epoch in range(n_epoch):
    ...
    for i in range(n_iter):
        ...
        for j in range(batchsize):
            x[j] = xp.asarray(Image.open(imagepaths[i*batchsize + j]).convert('RGB').resize((512,512)), dtype=np.float32).transpose(2, 0, 1)
