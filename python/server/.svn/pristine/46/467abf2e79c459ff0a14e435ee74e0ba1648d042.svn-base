...
parser.add_argument('--maxn', '-n', default=999999999, type=int)
...
style = vgg.preprocess(np.asarray(Image.open(args.style_image).convert('RGB').resize((1024,1024),2), dtype=np.float32))
...
for epoch in range(n_epoch):
    ...
    for i in range(n_iter):
        ...
        x = xp.zeros((batchsize, 3, 1024, 1024), dtype=xp.float32)
        for j in range(batchsize):
            # x[j] = xp.asarray(Image.open(imagepaths[i*batchsize + j]).convert('RGB').resize((1024,1024),2), dtype=np.float32).transpose(2, 0, 1)
            x[j] = xp.asarray(ImageOps.fit(Image.open(imagepaths[i*batchsize + j]).convert('RGB'), (1024,1024), Image.ANTIALIAS), dtype=np.float32).transpose(2, 0, 1)
